...
TeamGenerator.prototype.init = function () {
	...
	var coachingSpecialties = ["Bench", "Base", "Hitting", "Pitching", "Strength & Conditioning"];
...
TeamGenerator.prototype.bindEvents = function (){
	$(".create-team").on(
		"click",
		$.proxy(this.generateTeam, this)
	);
};

TeamGenerator.prototype.generateTeam = function() {
	...
	$('.boston').html(
		this.renderCoachesTable(boston.coaches)
	);
	$('.los-angeles').html(
		this.renderCoachesTable(losAngeles.coaches)
	);
};

TeamGenerator.prototype.generatePlayer = function(position){
	var first = Utility.randomFromArray(this.firstInitials);
	var last = Utility.randomFromArray(this.lastNames);
	var battingAvg = Math.round((.150 + Math.random()*.250)*1000)/1000;
	return this.generatePlayer(first, last, position, battingAvg);
};

TeamGenerator.prototype.generateCoach = function(specialty){
	var first = Utility.randomFromArray(this.firstInitials);
	var last = Utility.randomFromArray(this.lastNames);
	return this.generateCoach(first, last);
};

TeamGenerator.prototype.renderCoachesTable = function(coachesArray){
	var headers = "<tr><th>First</th><th>Last</th></tr>"
	var coaches = "";
	for (var i = 0; i < coachesArray.length; i++){
		var coach = "<tr><td>"+coachesArray[i].firstInitial+"</td><td>"+coachesArray[i].firstInitial+"</td></tr>"
...
TeamGenerator.prototype.renderPlayersTable = function(playersArray){
	var htmlString = "";
	return htmlString;
};
