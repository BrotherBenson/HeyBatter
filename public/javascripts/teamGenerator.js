...
TeamGenerator.prototype.bindEvents = function(){
	...
	$('.routing-button').on(
		'click',
		$.proxy(this.routing, this)
	);
};


TeamGenerator.prototype.routing = function(){
	$.ajax({
		method: "PUT",
		url: "/game", 
		data: { name: "PERSON", location: "NEVER"}
	});
};

TeamGenerator.prototype.generateTeam = function(city, division){
	...
	teamID++;
	return new Team(city, playerArray, bullpenArray, coachArray, teamID, division);
};
